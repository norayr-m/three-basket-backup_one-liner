...
**Bonus: [Volumetric Lattice Gas Automaton](volume.html)** — The hard one. Classical lattice gas automata leak energy through damping multipliers (typically 0.992× per tick). This one doesn't. It runs the exact same I/P lambda as the backup scheduler, but instead of rotating tar/rsync across cloud drives, it rotates absolute state snapshots and 6-neighbor kinetic deltas across three state reels in a 16³ tetrahedron lattice. The I-frame resets one reel to ground truth; the 11 P-frames propagate entropy between neighbors at zero loss. No damping. No artificial viscosity. Conservation is enforced by the structure, not patched after the fact. Bayer RGGB coloring maps `(x + y·2 + z·3) % 4` to red/blue/green channels — the same mod arithmetic that runs inside every camera sensor. Locks to a master clock via WebSocket for deterministic tick synchronization.
